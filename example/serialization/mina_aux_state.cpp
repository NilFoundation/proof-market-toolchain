...
zk::snark::proof_type<curve_type> make_proof(boost::property_tree::ptree root) {
    ...
    for (auto &evals_it : best_chain.second.get_child(base_path + "openings.evals")) {
        ...
        //        proof.evals[ev_i].generic_selector =
        //        get_cppui256(evals_it.second.get_child("generic_selector").begin());
        for (auto s_it : evals_it.second.get_child("generic_selector")) {
            proof.evals[ev_i].generic_selector.push_back(get_cppui256(&s_it));
        }

        //        proof.evals[ev_i].poseidon_selector =
        //        get_cppui256(evals_it.second.get_child("poseidon_selector").begin());
...
void prepare_proof_scalar(
    zk::snark::proof_type<nil::crypto3::algebra::curves::vesta> &original_proof,
    ...
    typename BlueprintFieldType::value_type random_value = algebra::random_element<BlueprintFieldType>();
...
std::string generate_proof_base(zk::snark::proof_type<nil::crypto3::algebra::curves::vesta> *pickles_proof,
    ...
    for (std::size_t batch_id = 0; batch_id < batch_size; batch_id++) {
        ...
        prepare_proof_base<curve_type, BlueprintFieldType, kimchi_params, eval_rounds>(kimchi_proof, proof,
                                                                                       public_input);
    }
    ...
    out.open("state_base.json");
    out_input.open("state_base_public_input.json");
...
std::string generate_proof_scalar(zk::snark::proof_type<nil::crypto3::algebra::curves::vesta> *pickles_proof,
    ...
    for (std::size_t batch_id = 0; batch_id < batch_size; batch_id++) {
        ...
        prepare_proof_scalar<curve_type, BlueprintFieldType, kimchi_params, eval_rounds>(kimchi_proof, proof,
                                                                                         public_input);
    }
    ...
    std::ofstream out;
    std::ofstream out_input;
    out.open("mina_aux_state.json");
    out_input.open("mina_aux_state_public_input.json");
    placeholder_proof_gen::serialize(out, desc, public_assignment, bp);
    boost::json::value jv = {
        {"public_input", public_input},
    };
...
int main(int argc, char *argv[]) {
    ...
    boost::property_tree::read_json(std::string(argv[1]), root);
    boost::property_tree::read_json(std::string(argv[2]), const_root);
