...
* Application on top of Ethereum: The interface that facilitates user interaction with the system. Users submit their requests for proofs, and once the proof is generated and validated, the proof generator gets paid.
* Drop DBMS: This is what bridges the gap between Ethereum and Proof Market. It transfers user requests to the Proof Market, updates order status, and submits the generated proofs to the Ethereum side.
* Proof System Verifiers: This is the system that verifies the incoming proofs on the Ethereum network.
* Proof Producers: They generate proofs and interact with the Proof Market as [usual](https://docs.nil.foundation/proof-market/market/user-guides/proof-producer).

Despite the default way of proof submission being via DROP DBMS, it is possible to send proofs to the Endpoint without it. 
Thus, proof producers can be sure that they will get paid for their work in any case.
