...
#include <nil/proof-generator-mt/aspects/mina_state.hpp>
...
namespace nil {
    namespace proof_generator_mt {
        namespace aspects {
            mina_state::mina_state(boost::shared_ptr<path> aspct) : path_aspect(std::move(aspct)) {
            }

            void mina_state::set_options(cli_options_type &cli) const {
                boost::program_options::options_description options("NIL Proof Generator");
                // clang-format off
                options.add_options()
                ("constants_input", boost::program_options::value<std::string>(), "Circuit input file");
                // clang-format on
                cli.add(options);
            }

            void mina_state::set_options(cfg_options_type &cfg) const {
                boost::program_options::options_description options("NIL Proof Generator");
                // clang-format off
                options.add_options()
            ...
            void mina_state::initialize(configuration_type &vm) {
            ...
            boost::filesystem::path prover::default_config_path() const {
                return path_aspect->config_path() / "config.ini";
            }

            std::string prover::input_circuit_string() const {
                return json_circuit;
            }

            std::string prover::input_public_params_string() const {
                return json_public_input;
            }

            std::string prover::output_file_string() const {
                return output_file;
            }
        }    // namespace aspects
    }        // namespace proof_generator_mt
